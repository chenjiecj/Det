...
#from nets.utils import _get_clones, inverse_sigmoid, multi_scale_deformable_attn_pytorch
...
if __name__ == "__main__":
    ...
    model = AIFI(256).to(device)
    # model = resnet18().to(device)
    # model = mobilev2().to(device)

    summary(model, (256, input_shape[0], input_shape[1]))

    dummy_input = torch.randn(1, 64, input_shape[0], input_shape[1]).to(device)
